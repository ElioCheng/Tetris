...
JBlock::JBlock() : name('J'), heavyBlock(false)  {
...
bool JBlock::getState() {
    return heavyBlock;
}

void JBlock::setState(bool state) {
    heavyBlock = state;
}

void JBlock::set(string judge, int index, int num) {
    if (judge == "X") {
...
int JBlock::get(string judge, int index) {
    if (judge == "X") {
...
bool JBlock::exist(int Xvalue, int Yvalue) {
    for (int i = 0; i < 4; i++) {
...
Block* JBlock::getCopy(Block* copy) {
    for (int i = 0; i < 4; i++) {
        int x = this->get("X", i);
        copy->set("X", i , x);
        int y = this->get("Y", i);
